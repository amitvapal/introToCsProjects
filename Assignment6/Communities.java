public class Communities {
    ...
    public static int numberOfCommunities (double[][] data, int columnIndex, double percentageThreshold, boolean isDisadvantage) {
        ...
        int counter = 0;
        double threshold = percentageThreshold / 100;
        int i = 0;
        while(i<data.length){
            double percentage = data[i][columnIndex];
            boolean isComDis = (data[i][21] == 1.0);
            if ((isDisadvantage && isComDis) || (!isDisadvantage && !isComDis)) {
    ...
    public static double largestRacialDemographic(double[][] data, int community) {
        ...
        double max = data[community][0];
        for(int i =0; i < data[community].length-14; i++){
    ...
    public static int mostPopulatedDisadvantagedWithLowIncome(double[][] data) {
        ...
        // the total number of people in the community.(11)
        // the percentage of the population that is low income (20)
        // the percentage of the population that is disadvantaged (21)
        double max = 0.0;
        int index = 0;
        for(int i =0; i < data.length; i++){
            if((data[i][20] == 1.0) && data[i][21] == 1.0){
                if(data[i][11] > max){
                    max = data[i][11];
                    index = i;
                }
