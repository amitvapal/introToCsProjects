...
public class DNA {
    ...
    public void createDatabaseOfProfiles ( String filename ) {
        ...
        int numProfiles = Integer.parseInt(StdIn.readLine());
        database = new Profile[numProfiles];

        //read number of profiles from input file
    ...
    public void readSTRsOfInterest ( String filename ) {
        ...
        /* WRITE YOUR CODE HERE */
        int numStr = Integer.parseInt(StdIn.readLine());
        STRsOfInterest = new String[numStr];
	...
	/* WRITE YOUR CODE HERE */
        ...
        Profile unknown = new Profile("Unknown", null, null, str1, str2);
    ...
    public STR findSTRInSequence ( String sequence, String STR ) {
        ...
        STR str = new STR(STR, n);
        return str; // update the return value
    }
         ...
         for ( int p1 = 0; p1 < possibleParent1.size(); p1++ ) {
            ...
            for ( int p2 = 0; p2 < possibleParent2.size(); p2++ ) {
                ...
                if ( !possibleParent1.get(p1).equals(possibleParent2.get(p2)) ) {
                    ...
                    if ( punnetSquare(possibleParent1.get(p1).getS1_STRs(), S2_STRs, possibleParent2.get(p2).getS1_STRs(), S1_STRs)) {
                        parentList.add(possibleParent1.get(p1));
                        parentList.add(possibleParent2.get(p2));
                    } else if (punnetSquare(possibleParent1.get(p1).getS2_STRs(), S2_STRs, possibleParent2.get(p2).getS1_STRs(), S1_STRs)) {
                        parentList.add(possibleParent1.get(p1));
                        parentList.add(possibleParent2.get(p2));
                    } else if (punnetSquare(possibleParent1.get(p1).getS1_STRs(), S1_STRs, possibleParent2.get(p2).getS2_STRs(), S2_STRs)) {
                        parentList.add(possibleParent1.get(p1));
                        parentList.add(possibleParent2.get(p2));
                    } else if (punnetSquare(possibleParent1.get(p1).getS2_STRs(), S2_STRs, possibleParent2.get(p2).getS2_STRs(), S1_STRs)) {
                        parentList.add(possibleParent1.get(p1));
                        parentList.add(possibleParent2.get(p2));
                    }
                }
